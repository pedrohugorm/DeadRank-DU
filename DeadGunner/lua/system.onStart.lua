...
function initializeDpsHistory(data)
    for i=1,maxChartPointCount do 
        table.insert(data, 0)
    end
end

function addDps(previousDps, damage)
    return previousDps + damage
end

function pushDpsToHistory(data, damage)
    table.insert(data, damage)
    
    if #data > maxChartPointCount then
        popDpsFromHistory(data)
    end
end

function popDpsFromHistory(data) 
    return table.remove(data, 1)
end

function getMaxValueFromDps()
    return max({ max(selfDpsHistory), max(enemyDpsHistory) })
end

function max(arrayOfNum)
    local maxVal = 100;

    for _,v in pairs(arrayOfNum) do
        if v > maxVal then
            maxVal = v
        end
    end
...
function historyToPoints(mv, mh, maxDps, history)
    ...
    for k,v in ipairs(history) do
        local x = lerp(0, mh, i / length)
        local y = lerp(mv, 0, v / maxDps)
...
function renderDpsChart()
    local maxValueVertical = 120
    local maxValueHorizontal = 450
    local maxDps = getMaxValueFromDps()
    
    selfPoints = historyToPoints(maxValueVertical, maxValueHorizontal, maxDps, selfDpsHistory)
    enemyPoints = historyToPoints(maxValueVertical, maxValueHorizontal, maxDps, enemyDpsHistory)
    
    html = [[ 
        <div style="width: 12.5%; height: 12.5%; position: absolute;left:3%;top:5%; background-color: rgba(255, 255, 255, 0.1); padding: 1vh">
